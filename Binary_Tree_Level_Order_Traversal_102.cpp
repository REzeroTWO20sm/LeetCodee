...
// g++ Binary_Tree_Level_Order_Traversal_102.cpp -o Binary_Tree_Level_Order_Traversal_102 --std=c++14 -I/usr/local/include
...
std::vector<std::vector<int>> levelOrder(TreeNode* root) {
    ...
    if (root->left != nullptr || root->right != nullptr) {
        ...
        if (root->left != nullptr) {
            levelValues.push_back(root->left->val);
        }

        if (root->right != nullptr) {
            levelValues.push_back(root->right->val);
        }

        vec.push_back(levelValues);
    }

    std::vector<std::vector<int>> leftVec;
    if (root->left != nullptr) {
        leftVec = levelOrder(root->left);
    }

    std::vector<std::vector<int>> rightVec;
    if (root->right != nullptr) {
        rightVec = levelOrder(root->right);
    }
...
int main(){
    ...
    std::vector<std::vector<int>> vec_tree = levelOrder(root_2);
    vec_tree.insert(vec_tree.begin(), {1});
    for(int i=0;i<vec_tree.size();i++){
        std::cout << "[";
        for(auto it : vec_tree[i]){
            std::cout << it << ",";
        }
        std::cout << "] ";
    }
